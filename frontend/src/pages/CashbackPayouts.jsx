      ...
      <div className="ml-0 lg:ml-72 min-h-screen relative w-full">
        ...
        <div className="sticky top-0 z-[100] bg-white/80 backdrop-blur-xl border-b border-slate-100 px-6 py-4 flex justify-between items-center">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-xl font-black uppercase italic tracking-tighter text-blue-900 leading-none">Cashback Portal</h1>
            ...
            <div className="flex items-center gap-3">
                ...
                <div className="hidden sm:flex items-center gap-2 px-3 py-1 bg-amber-50 border border-amber-100 rounded-full">
                    ...
                    <span className="text-[7px] font-black uppercase tracking-widest text-amber-600 italic">Live Sync Active</span>
                </div>
                <button 
                    onClick={handleGeneratePayouts}
                    disabled={processing}
                    className="hidden md:flex items-center gap-2 bg-blue-900 text-white px-6 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-[0.2em] italic hover:bg-amber-600 transition-all shadow-lg active:scale-95 disabled:opacity-50"
                >
        ...
        <main className="p-6 md:p-10 w-full max-w-[1600px] space-y-10 pb-32">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
            <div>
              <h2 className="text-4xl md:text-6xl font-black text-blue-900 tracking-tighter uppercase italic leading-none">Yield Disbursements</h2>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              { label: 'Current Month Total', value: stats?.monthly || 0, icon: DollarSign, color: 'text-amber-600', bg: 'bg-amber-50' },
              { label: 'Active Yield Cycles', value: stats?.active_cycles || 0, icon: RefreshCw, color: 'text-blue-600', bg: 'bg-blue-50' },
              { label: 'Total Cumulative Yield', value: stats?.total_paid || 0, icon: ArrowUpRight, color: 'text-blue-600', bg: 'bg-blue-50' },
            ].map((stat, i) => (
              <motion.div 
                key={i} 
              ...
              >
                <div className={`w-12 h-12 ${stat.bg} rounded-2xl flex items-center justify-center mb-6 group-hover:bg-blue-900 transition-all duration-500`}>
                ...
                <h3 className="text-2xl font-black text-blue-900 italic tracking-tighter">
                    ...
                    >
                      <td className="py-8 px-10">
                        <div className="flex items-center gap-6">
                          ...
                          <div>
                            <span className="text-base font-black text-blue-900 italic uppercase tracking-tighter block">{p.user_name}</span>
                        ...
                        {p.bank_name ? (
                            <div className="space-y-1">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-[10px] font-black text-blue-900 uppercase italic">{p.bank_name}</span>
                                </div>
                      ...
                      <td className="py-8 px-8 text-center">
                         <div className="inline-flex items-center gap-2">
                            <span className={`px-3 py-1 rounded-full text-[7px] font-black uppercase tracking-widest italic border ${
                                p.status === 'completed' ? 'bg-amber-50 text-amber-600 border-amber-100' : 
                            ...
                            {p.status === 'completed' && <CheckCircle2 size={14} className="text-amber-500" />}
                         </div>
                      </td>
                        ...
                        <div className="flex items-center justify-end gap-2">
                            {p.status === 'pending' && (
                                <button 
                                    ...
                                    className="bg-amber-500 text-white px-5 py-2.5 rounded-xl text-[8px] font-black uppercase tracking-widest italic hover:bg-amber-600 transition-all shadow-lg active:scale-95"
                                >
                                    Mark Paid
                                </button>
                            )}
                            <button 
                                ...
                                className="w-10 h-10 flex items-center justify-center bg-white border border-slate-200 rounded-xl text-slate-400 hover:bg-blue-900 hover:text-white transition-all shadow-sm"
