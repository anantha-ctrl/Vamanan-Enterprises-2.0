...
const StatusBadge = ({ status }) => {
  const map = {
    ...
    active:   { bg: 'bg-amber-50',text: 'text-amber-600',border: 'border-amber-200',icon: <CheckCircle size={10} />,  label: 'Active'   },
    rejected: { bg: 'bg-blue-50',   text: 'text-blue-600',   border: 'border-blue-200',   icon: <XCircle size={10} />,      label: 'Rejected' },
...
const Shop = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex font-inter text-blue-900 overflow-x-hidden selection:bg-amber-100 selection:text-amber-900">
        ...
        <main className="p-4 sm:p-6 md:p-8 lg:p-10 xl:p-12 w-full max-w-[1500px] space-y-8 md:space-y-10 pb-32 lg:pb-16">
          ...
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div>
              ...
              <h1 className="text-3xl md:text-5xl font-black text-blue-900 tracking-tighter uppercase italic leading-none">Buy Gold &amp; Silver</h1>
            ...
            <div className="flex items-center gap-3">
              {activeTab === 'products' && cartCount > 0 && (
                <button onClick={() => setShowCart(true)}
                  className="relative bg-blue-900 text-white px-5 py-3 rounded-2xl text-[9px] font-black uppercase tracking-widest flex items-center gap-2 shadow-xl hover:bg-amber-600 transition-all">
                  <ShoppingCart size={14} /> Cart
                  <span className="absolute -top-2 -right-2 w-5 h-5 bg-amber-500 rounded-full text-[9px] font-black flex items-center justify-center text-white">{cartCount}</span>
                </button>
              )}
          ...
          <div className="flex gap-3">
            {[
              { key: 'gold',     label: 'Gold 22K',    icon: <Zap size={16} />,      active: 'bg-blue-900 text-amber-500 border-blue-900 shadow-xl', inactive: 'hover:border-amber-200' },
              { key: 'silver',   label: 'Silver Pure', icon: <Coins size={16} />,    active: 'bg-blue-900 text-slate-200 border-blue-900 shadow-xl', inactive: 'hover:border-slate-300' },
              { key: 'products', label: 'Products',    icon: <Package size={16} />,  active: 'bg-blue-900 text-amber-400 border-blue-900 shadow-xl', inactive: 'hover:border-amber-200' },
            ].map(tab => (
              <button key={tab.key}
                ...
                {tab.key === 'products' && cartCount > 0 && (
                  <span className="absolute -top-1.5 -right-1.5 w-4 h-4 bg-amber-500 rounded-full text-[8px] font-black flex items-center justify-center text-white">{cartCount}</span>
                )}
              </button>
            ))}
          </div>
            ...
            <>
              <motion.div key={assetType} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}
                ...
                <div>
                  ...
                  <h2 className="text-3xl md:text-5xl font-black text-blue-900 tracking-tighter italic">₹{currentPrice.toLocaleString()} <span className="text-sm md:text-lg text-slate-400">/ gram</span></h2>
                  <div className="flex flex-wrap items-center gap-4 mt-4 md:mt-6">
                    <div className="bg-amber-50 text-amber-600 px-4 md:px-6 py-2 rounded-full text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] border border-amber-100 italic flex items-center gap-2">
                ...
                <div className={`w-20 h-20 md:w-32 md:h-32 bg-slate-50 rounded-2xl md:rounded-[2rem] flex items-center justify-center border border-slate-100 shadow-inner group-hover:bg-blue-900 transition-all duration-500 ${assetType === 'gold' ? 'text-amber-500 group-hover:text-amber-500' : 'text-slate-400 group-hover:text-slate-200'}`}>
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
                ...
                <div className="p-8 md:p-16">
                  <div className="flex items-center gap-4 border-b border-slate-100 pb-6 md:pb-8 mb-8 md:mb-12">
                    <div className="w-10 h-10 bg-blue-900 rounded-xl flex items-center justify-center text-amber-500 shadow-lg"><Zap size={20} /></div>
                    <h3 className="text-[9px] md:text-[10px] font-black text-blue-900 uppercase tracking-[0.3em] italic">Enter Amount</h3>
                  </div>
                  <div className="mb-8 md:mb-12">
                    ...
                    <div className="relative group">
                      <input type="number" className="w-full bg-slate-50 border border-slate-200 rounded-2xl md:rounded-[2rem] px-8 md:px-10 py-6 md:py-8 text-blue-900 focus:border-amber-500 focus:bg-white outline-none text-2xl md:text-3xl font-black transition-all shadow-inner italic"
                  ...
                  <div className="space-y-6 md:space-y-8 pt-8 border-t border-slate-50 mb-8">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-lg md:text-xl font-black text-blue-900 italic">₹{baseAmount.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}</span>
                    </div>
                    <div className="flex justify-between items-center">
                    ...
                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center pt-6 border-t border-slate-100 gap-4">
                      <span className="text-base md:text-lg font-black text-blue-900 uppercase tracking-[0.3em] italic">Total Amount</span>
                  ...
                  <button onClick={handleGoldSilverBuy} disabled={buying}
                    className="w-full bg-blue-900 text-white py-6 md:py-8 rounded-2xl md:rounded-[2rem] font-black text-[9px] md:text-[10px] uppercase tracking-[0.3em] hover:bg-amber-600 transition-all shadow-2xl active:scale-95 italic group flex items-center justify-center gap-3">
          ...
          {activeTab === 'products' && (
            <motion.div key="products-panel" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-10">
              ...
              {categories.length > 0 && (
                <div className="flex gap-2 flex-wrap">
                  <button
                    ...
                    className={`py-2.5 px-5 rounded-2xl font-black text-[9px] uppercase tracking-widest italic transition-all border-2 ${activeCategory === 'all' ? 'bg-blue-900 text-amber-400 border-blue-900 shadow-lg' : 'bg-white text-slate-400 border-slate-100 hover:border-slate-300'}`}>
                    All Products
                  </button>
                  {categories.map(cat => (
                    <button key={cat.id}
                      onClick={() => setActiveCategory(cat.name)}
                      className={`py-2.5 px-5 rounded-2xl font-black text-[9px] uppercase tracking-widest italic transition-all border-2 ${activeCategory === cat.name ? 'bg-blue-900 text-amber-400 border-blue-900 shadow-lg' : 'bg-white text-slate-400 border-slate-100 hover:border-slate-300'}`}>
                      {cat.name}
                    </button>
                  ))}
                </div>
                    ...
                    return (
                      <motion.div key={product.id}
                        ...
                        className={`flex flex-col bg-white rounded-[2rem] overflow-hidden border-2 shadow-sm transition-all duration-300 ${
                          isOutOfStock ? 'border-blue-100 opacity-75'
                          : inCart ? 'border-amber-400 shadow-amber-100 shadow-lg'
                        ...
                        <div className="relative h-52 bg-slate-50 overflow-hidden shrink-0 flex items-center justify-center relative">
                          ...
                          {product.purity && (
                            <div className={`absolute top-3 left-3 text-[8px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full shadow ${/^(22K|24K|18K|999)$/.test(product.purity) ? 'bg-amber-500 text-blue-900' : 'bg-blue-800 text-white'}`}>
                              {product.purity}
                            </div>
                          )}
                          ...
                          {inCart && !isOutOfStock && (
                            <div className="absolute top-3 right-3 bg-amber-500 text-white text-[8px] font-black uppercase tracking-widest italic px-3 py-1.5 rounded-full shadow-lg flex items-center gap-1">
                          ...
                          <div className="absolute bottom-0 left-0 right-0">
                            {isOutOfStock && (
                              <div className="bg-blue-600/90 backdrop-blur-sm text-white text-[9px] font-black uppercase tracking-widest py-2 text-center flex items-center justify-center gap-1.5">
                                <X size={11} strokeWidth={3} /> Out of Stock
                              </div>
                            )}
                            {isLowStock && (
                              <div className="bg-amber-500/90 backdrop-blur-sm text-blue-900 text-[9px] font-black uppercase tracking-widest py-2 text-center flex items-center justify-center gap-1.5">
                                <AlertCircle size={11} strokeWidth={2.5} /> Only {stockQty} left — Hurry!
                              </div>
                            )}
                            {isInStock && (
                              <div className="bg-amber-500/80 backdrop-blur-sm text-white text-[8px] font-black uppercase tracking-widest py-1.5 text-center flex items-center justify-center gap-1.5">
                        ...
                        <div className="flex flex-col flex-1 p-5 gap-4">
                          {/* Name + badges */}
                          <div>
                            <h4 className={`text-sm font-black uppercase italic tracking-tight leading-tight mb-2 ${isOutOfStock ? 'text-slate-400' : 'text-blue-900'}`}>
                              {product.name}
                            </h4>
                          ...
                          <div className={`rounded-2xl p-4 space-y-2 border ${isOutOfStock ? 'bg-slate-50 border-slate-100' : 'bg-slate-50 border-slate-100'}`}>
                            <div className="flex justify-between items-center">
                              ...
                              <span className={`text-base font-black italic tracking-tighter ${isOutOfStock ? 'text-slate-400' : 'text-blue-900'}`}>
                            ...
                            {isOutOfStock && (
                              <div className="flex items-center justify-center gap-2 pt-1 border-t border-slate-200">
                                <span className="text-[8px] font-black text-blue-400 uppercase tracking-widest italic">Currently Unavailable</span>
                              </div>
                            )}
                          </div>
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div className="flex items-center bg-slate-50 border border-slate-200 rounded-2xl overflow-hidden flex-1">
                                <button onClick={() => setQty(product.id, qty - 1)}
                                  className="px-3 py-2.5 hover:bg-slate-200 transition-colors text-slate-500 hover:text-blue-900">
                                  <Minus size={12} strokeWidth={3} />
                                </button>
                                <span className="flex-1 text-center text-sm font-black text-blue-900 italic">{qty}</span>
                                <button onClick={() => setQty(product.id, qty + 1)}
                                  className="px-3 py-2.5 hover:bg-slate-200 transition-colors text-slate-500 hover:text-blue-900">
                            ...
                            {isOutOfStock ? (
                              ...
                              <>
                                <button onClick={() => addToCart(product)}
                                  className={`flex-1 py-3 rounded-xl font-black text-[8px] uppercase tracking-widest italic transition-all active:scale-95 flex items-center justify-center gap-1.5 border-2 ${inCart ? 'bg-amber-50 text-amber-600 border-amber-200 hover:bg-amber-100' : 'bg-white text-slate-700 border-slate-200 hover:border-blue-900 hover:text-blue-900'}`}>
                                  <ShoppingCart size={11} />
                                  {inCart ? 'Add More' : 'Add to Cart'}
                                </button>
                                <button onClick={() => handleSingleBuy(product)}
                                  className={`flex-1 py-3 rounded-xl font-black text-[8px] uppercase tracking-widest italic transition-all active:scale-95 flex items-center justify-center gap-1.5 ${isLowStock ? 'bg-amber-500 text-blue-900 hover:bg-amber-400' : 'bg-blue-900 text-white hover:bg-amber-600'}`}>
              ...
              {cart.length > 0 && (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
                  className="bg-blue-900 text-white rounded-[2rem] p-6 md:p-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-5 shadow-2xl">
                  ...
                  <div className="flex gap-3 w-full sm:w-auto">
                    ...
                    <button onClick={handleCartCheckout}
                      className="flex-1 sm:flex-none px-6 py-3 bg-amber-500 text-blue-900 rounded-xl text-[9px] font-black uppercase tracking-widest italic hover:bg-amber-400 transition-all active:scale-95 flex items-center gap-2 justify-center shadow-lg">
              ...
              <div>
                <div className="flex items-center gap-3 mb-5">
                  <div className="w-9 h-9 bg-blue-900 rounded-xl flex items-center justify-center text-amber-500 shadow-lg shrink-0"><ClipboardList size={18} /></div>
                  <div>
                    <h3 className="text-[10px] md:text-xs font-black text-blue-900 uppercase tracking-[0.3em] italic">My Orders</h3>
                        ...
                        <motion.div key={order.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}
                          ...
                          <div className="w-12 h-12 rounded-xl bg-slate-50 border border-slate-100 shrink-0 flex items-center justify-center">
                            {assetType === 'silver'
                              ...
                              : assetType === 'product'
                                ? <Package size={22} className="text-amber-500" strokeWidth={1.5} />
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex flex-wrap items-center gap-2 mb-1">
                              <p className="text-xs font-black text-blue-900 uppercase italic truncate">{assetLabel}</p>
                            ...
                            <div className="flex flex-wrap gap-x-4 gap-y-1">
                              ...
                              {order.daily_payout > 0 && (
                                <p className="text-[8px] text-amber-500 font-bold uppercase tracking-widest italic">
                            ...
                            {order.status === 'completed' ? (
                              <p className="text-[8px] text-amber-500 font-bold uppercase tracking-widest italic mt-1.5 flex items-center gap-1">
        ...
        {showCart && (
          <>
            <motion.div key="cart-overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              className="fixed inset-0 bg-blue-900/60 backdrop-blur-sm z-[250]" onClick={() => setShowCart(false)} />
            <motion.div key="cart-drawer"
              ...
              <div className="flex items-center justify-between p-6 border-b border-slate-100">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 bg-blue-900 rounded-xl flex items-center justify-center text-amber-500"><ShoppingCart size={18} /></div>
                  <div>
                    <h2 className="text-sm font-black text-blue-900 uppercase italic tracking-tighter">Your Cart</h2>
                    <p className="text-[8px] text-slate-400 font-black uppercase tracking-widest italic">{cartCount} item{cartCount !== 1 ? 's' : ''}</p>
                  </div>
                </div>
                <button onClick={() => setShowCart(false)} className="p-2 hover:bg-slate-100 rounded-xl transition-colors text-slate-400 hover:text-blue-900"><X size={18} /></button>
              </div>
                ...
                {cart.length === 0 ? (
                  ...
                  return (
                    <div key={item.id} className="bg-slate-50 rounded-2xl p-4 flex items-center gap-3 border border-slate-100">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-black text-blue-900 uppercase italic truncate">{item.name}</p>
                      ...
                      <div className="flex flex-col items-end gap-2 shrink-0">
                        <p className="text-sm font-black text-blue-900 italic">₹{itemTotal.toLocaleString()}</p>
                        <div className="flex items-center bg-white border border-slate-200 rounded-xl overflow-hidden">
                          ...
                          <span className="px-2 text-xs font-black text-blue-900">{item.quantity}</span>
                      ...
                      <button onClick={() => removeFromCart(item.id)} className="p-1.5 text-slate-300 hover:text-blue-500 hover:bg-blue-50 rounded-lg transition-all ml-1"><Trash2 size={13} /></button>
                    </div>
                  );
                })}
              </div>
              ...
              {cart.length > 0 && (
                <div className="border-t border-slate-100 p-5 space-y-4">
                  <div className="space-y-2">
                    ...
                    <div className="flex justify-between text-sm font-black text-blue-900 uppercase italic tracking-tighter border-t border-slate-100 pt-2">
                  ...
                  <button onClick={() => { setShowCart(false); handleCartCheckout(); }}
                    className="w-full bg-blue-900 text-white py-4 rounded-2xl font-black text-[9px] uppercase tracking-[0.3em] italic hover:bg-amber-600 transition-all active:scale-95 flex items-center justify-center gap-2 shadow-xl">
                    Checkout <ArrowRight size={16} />
                  </button>
                  <button onClick={() => setCart([])}
                    className="w-full py-3 border border-blue-200 text-blue-500 rounded-2xl font-black text-[8px] uppercase tracking-widest italic hover:bg-blue-50 transition-all flex items-center justify-center gap-2">
      ...
      <AnimatePresence>
        {showPaymentModal && (
          <div key="payment-overlay" className="fixed inset-0 bg-blue-900/90 backdrop-blur-xl z-[300] flex items-center justify-center md:p-6 overflow-y-auto">
            <motion.div key="payment-modal-content"
              ...
              <button onClick={() => setShowPaymentModal(false)}
                className="absolute top-6 right-6 md:top-8 md:right-8 z-50 p-2 md:p-3 bg-slate-100/10 hover:bg-slate-100 rounded-2xl text-white md:text-slate-400 hover:text-blue-900 transition-colors backdrop-blur-md">
                <X size={20} />
              </button>

              {/* Left: Payment details */}
              <div className="w-full md:w-1/2 bg-blue-900 p-8 sm:p-12 md:p-16 lg:p-20 flex flex-col items-center justify-center text-center relative overflow-hidden shrink-0">
                ...
                <div className="relative z-10 w-full max-w-[280px] mt-4 md:mt-0">
                  ...
                  <div className="flex bg-white/5 border border-white/10 p-1 rounded-2xl mb-8">
                    <button onClick={() => setPaymentMode('bank')}
                      className={`flex-1 py-2 text-[10px] font-black uppercase tracking-wider italic rounded-xl transition-all ${paymentMode === 'bank' ? 'bg-amber-500 text-blue-900 shadow-lg' : 'text-slate-400 hover:text-white'}`}>Bank Transfer</button>
                    <button onClick={() => setPaymentMode('upi')}
                      className={`flex-1 py-2 text-[10px] font-black uppercase tracking-wider italic rounded-xl transition-all ${paymentMode === 'upi' ? 'bg-amber-500 text-blue-900 shadow-lg' : 'text-slate-400 hover:text-white'}`}>UPI Scan</button>
                  </div>
                      ...
                      <div className="w-full bg-white p-5 md:p-7 rounded-[2rem] shadow-[0_0_50px_rgba(245,158,11,0.2)] mb-6 text-left space-y-3 hover:scale-105 transition-transform duration-500">
                        {[
                          ...
                          <div key={row.label}>
                            ...
                            <p className="text-xs text-blue-900 font-extrabold uppercase">{row.val}</p>
                        ...
                        <div className="flex items-center justify-between gap-3">
                          <div>
                            ...
                            <p className="text-sm text-blue-900 font-black tracking-wider break-all">{paymentGateway.bankAccountNo || '638492017451'}</p>
                          </div>
                          <button onClick={() => handleCopy(paymentGateway.bankAccountNo || '638492017451', 'account')}
                            className="p-2 bg-slate-50 hover:bg-amber-500 hover:text-blue-900 rounded-xl transition-all text-slate-500 border border-slate-100 active:scale-95">
                            {copiedField === 'account' ? <span className="text-[8px] font-black uppercase italic px-1 text-amber-600">Copied</span> : <Copy size={12} />}
                          </button>
                        </div>
                        ...
                        <div className="flex items-center justify-between gap-3">
                          ...
                          <button onClick={() => handleCopy(paymentGateway.bankIfsc || 'CNRB0002987', 'ifsc')}
                            className="p-2 bg-slate-50 hover:bg-amber-500 hover:text-blue-900 rounded-xl transition-all text-slate-500 border border-slate-100 active:scale-95">
                            {copiedField === 'ifsc' ? <span className="text-[8px] font-black uppercase italic px-1 text-amber-600">Copied</span> : <Copy size={12} />}
                          </button>
                        </div>
                        {paymentGateway.bankBranch && (
                          ...
                          <div>
                            ...
                            <p className="text-xs text-blue-900 font-extrabold uppercase">{paymentGateway.bankBranch}</p>
                          </div></>
                        )}
                      </div>
              ...
              <div className="w-full md:w-1/2 p-8 md:p-16 overflow-y-auto max-h-[60vh] md:max-h-[85vh]">
                <div className="mb-8">
                  <h2 className="text-xl md:text-2xl font-black text-blue-900 uppercase italic tracking-tighter mb-1">Confirm Your Payment</h2>
                  ...
                  <div className="mb-6 bg-slate-50 rounded-2xl p-4 space-y-2 border border-slate-100 max-h-32 overflow-y-auto">
                    {cart.map(item => (
                      <div key={item.id} className="flex justify-between items-center text-[9px]">
                        ...
                        <span className="font-black text-blue-900 italic shrink-0">₹{(parseFloat(item.price)*item.quantity).toLocaleString()}</span>
                      </div>
                    ))}
                  </div>
                )}
                ...
                <form onSubmit={handleFinalSubmit} className="space-y-5">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <input type="text" required placeholder="Mobile Number"
                        ...
                        readOnly={!!user.phone}
                        className={`w-full ${user.phone ? 'bg-slate-50 text-slate-400' : 'bg-white text-blue-900 border-amber-200 focus:border-amber-500'} border rounded-xl py-4 px-5 text-sm font-black uppercase italic outline-none transition-all`} />
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <input type="text" required placeholder="Enter UTR / Reference Number"
                      ...
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl py-4 px-5 text-sm font-black text-blue-900 uppercase italic outline-none focus:border-amber-500 focus:bg-white transition-all shadow-inner" />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div>
                      ...
                      <label htmlFor="pay-proof"
                        ...
                        <ShieldCheck size={16} className={paymentForm.screenshot ? 'text-amber-500' : 'text-slate-300'} />
                      </label>
                    </div>
                  </div>

                  <button type="submit" disabled={buying}
                    className="w-full bg-blue-900 text-white py-5 rounded-2xl font-black text-[9px] uppercase tracking-[0.4em] italic shadow-2xl hover:bg-amber-600 transition-all active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-3 mt-2">
        ...
        {/* Status popup */}
        {status.message && (
          <div className="fixed inset-0 bg-blue-900/80 backdrop-blur-md z-[400] flex items-center justify-center p-6">
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0 }}
              ...
              <div className={`absolute top-0 left-0 w-full h-2 ${status.type === 'success' ? 'bg-amber-500' : 'bg-blue-500'}`} />
              <div className={`w-24 h-24 rounded-[2rem] mx-auto mb-8 flex items-center justify-center shadow-2xl ${status.type === 'success' ? 'bg-amber-50 text-amber-600' : 'bg-blue-50 text-blue-600'}`}>
                {status.type === 'success' ? <CheckCircle2 size={48} strokeWidth={1.5} /> : <AlertCircle size={48} strokeWidth={1.5} />}
              </div>
              <h3 className="text-2xl md:text-4xl font-black text-blue-900 mb-4 tracking-tighter uppercase italic">{status.type === 'success' ? 'Order Submitted!' : 'Error Detected'}</h3>
              ...
              <button onClick={() => setStatus({ type: '', message: '' })}
                className="w-full bg-blue-900 text-white py-5 rounded-2xl font-black text-[10px] uppercase tracking-[0.3em] hover:bg-amber-600 transition-all shadow-2xl italic active:scale-[0.98]">
                Acknowledge &amp; Close
              </button>
            </motion.div>
          </div>
