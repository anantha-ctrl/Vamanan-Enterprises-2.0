...
const LEVEL_STYLES = [
  { bg: 'bg-slate-900',  text: 'text-white',       ring: 'border-slate-800', label: 'bg-amber-500 text-slate-900' },
  { bg: 'bg-slate-800',  text: 'text-white',       ring: 'border-slate-700', label: 'bg-blue-500 text-white' },
        ...
        <main className="p-4 sm:p-6 md:p-8 lg:p-10 xl:p-12 w-full max-w-[1500px] space-y-8 md:space-y-12 pb-32 lg:pb-16">
            ...
            <div className="relative z-10 flex flex-col xl:flex-row xl:items-center gap-12 xl:gap-24">
              <div className="flex-1">
                ...
                <div className="relative inline-block mb-8 group/code w-full">
                  <h2 
                    onClick={copyLink}
                    className="text-4xl sm:text-6xl md:text-9xl font-black text-slate-900 tracking-tighter italic uppercase leading-none cursor-pointer hover:text-amber-500 transition-colors duration-500 select-none break-all"
                  >
                 ...
                 <div className="relative z-10 space-y-8 sm:space-y-10">
                    ...
                    <div className="grid grid-cols-2 gap-4 sm:gap-8">
                       <div>
                         ...
                         <h3 className="text-xl sm:text-2xl font-black text-emerald-600 italic tracking-tighter leading-none">+₹{parseFloat(todayEarnings).toLocaleString()}</h3>
                       </div>
                       <div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
            {[
              ...
              { label: 'Network Strength',   value: `${totalNodes} Members`, icon: ShieldCheck, color: 'text-emerald-600', bg: 'bg-emerald-50' },
              { label: 'Total Earnings',   value: `₹${parseFloat(totalEarnings).toLocaleString()}`, icon: Gift, color: 'text-rose-600', bg: 'bg-rose-50' },
            ].map((s, i) => (
              ...
              >
                <div className={`w-10 h-10 sm:w-12 sm:h-12 ${s.bg} rounded-xl sm:rounded-2xl flex items-center justify-center mb-4 group-hover:bg-slate-900 transition-all duration-500`}>
                ...
                <div>
                  ...
                  <h3 className="text-lg sm:text-xl md:text-2xl font-black text-slate-900 italic tracking-tighter">{s.value}</h3>
                </div>
              </motion.div>
            ))}
          </div>
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 mb-8 relative z-10">
              ...
              <div className={`flex items-center gap-2 px-6 py-3 rounded-full text-[10px] font-black uppercase tracking-widest border italic transition-all ${progress >= 10 ? 'bg-rose-50 text-rose-600 border-rose-100' : 'bg-emerald-50 text-emerald-600 border-emerald-100'}`}>
                {progress >= 10 ? (
            ...
            <div className="relative z-10">
              <div className="flex justify-between text-[9px] font-black uppercase tracking-widest text-slate-400 mb-3 italic">
                ...
                <span className={progress >= 10 ? 'text-rose-600' : 'text-amber-600'}>
              ...
              <div className="w-full h-4 bg-slate-100 rounded-full overflow-hidden border border-slate-200 shadow-inner p-1">
                <motion.div
                  ...
                  className={`h-full rounded-full relative transition-colors duration-700 ${progress >= 10 ? 'bg-rose-500' : 'bg-slate-900'}`}
                >
              ...
              <div className="flex gap-3 flex-wrap mt-5">
                {Array.from({ length: 10 }).map((_, i) => (
                  <div key={i} className={`w-8 h-2 rounded-full transition-all duration-700 ${i < progress ? (progress >= 10 ? 'bg-rose-500' : 'bg-slate-900') : 'bg-slate-100'}`} />
                ))}
              </div>
            </div>
          </motion.div>
                      ...
                      <div className="flex items-center gap-5">
                        ...
                        <div>
                          <div className="flex items-center gap-2 mb-1">
                            <h4 className="text-sm font-black text-slate-900 uppercase italic tracking-tight">Level {lvl.level}</h4>
                          ...
                          <div className="px-8 md:px-12 py-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                            {lvl.users.map((u, i) => (
                              <div key={i} className="flex items-center gap-3 bg-white border border-slate-100 rounded-2xl p-3 shadow-sm">
                                ...
                                <div>
                                  <p className="text-[10px] font-black text-slate-900 uppercase italic truncate">{u.name}</p>
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-10 relative z-10">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 bg-slate-900 rounded-2xl flex items-center justify-center text-amber-500 shadow-xl"><Users size={24} /></div>
                <div>
                  <h3 className="text-xl font-black uppercase italic tracking-tighter text-slate-900">Direct Members (Level 1)</h3>
              ...
              {directReferrals.length > 0 ? directReferrals.map((ref, idx) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-3 sm:gap-5">
                    <div className="w-10 h-10 sm:w-12 sm:h-12 bg-white rounded-xl sm:rounded-2xl flex items-center justify-center text-slate-900 border border-slate-100 shadow-sm group-hover/node:bg-slate-900 group-hover/node:text-amber-500 transition-all duration-500">
                       <User size={16} />
                    </div>
                    <div>
                      <p className="text-sm sm:text-base font-black italic uppercase tracking-tight text-slate-900 group-hover/node:text-amber-600 transition-colors">{ref.name}</p>
                      <p className="text-[8px] sm:text-[9px] text-slate-400 font-black uppercase tracking-widest mt-0.5 sm:mt-1">Active: {new Date(ref.created_at).toLocaleDateString('en-GB', { day:'2-digit', month:'short' })}</p>
                    </div>
                  </div>
                  <div className="text-left sm:text-right shrink-0 w-full sm:w-auto pt-3 sm:pt-0 border-t sm:border-t-0 border-slate-100">
                    ...
                    <p className={`text-base sm:text-lg font-black italic ${parseFloat(ref.invested) > 0 ? 'text-emerald-600' : 'text-slate-400'}`}>
          ...
          {history.length > 0 && (
            ...
            >
              <div className="p-8 md:p-12 border-b border-slate-100 flex items-center gap-4">
                <div className="w-10 h-10 bg-slate-900 rounded-xl flex items-center justify-center text-amber-500"><Gift size={20} /></div>
                <div>
                  <h3 className="text-sm font-black text-slate-900 uppercase italic tracking-tight">Referral Earnings History</h3>
                ...
                {history.map((h, i) => (
                  <div key={i} className="flex items-center justify-between px-8 md:px-12 py-5 hover:bg-slate-50/50 transition-colors group">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-[10px] font-black text-slate-900 uppercase italic">{h.description}</p>
                    ...
                    <p className="text-base font-black text-emerald-600 italic tracking-tighter group-hover:text-amber-600 transition-colors">
