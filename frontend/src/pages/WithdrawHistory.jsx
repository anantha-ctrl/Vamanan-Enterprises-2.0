...
const WithdrawHistory = () => {
  ...
  const handleWithdraw = async (e) => {
    ...
    if (parseFloat(form.amount) > data.balance) {
      alert("Insufficient Institutional Capital");
      return;
    }
    if (parseFloat(form.amount) < 1) {
      alert("Minimum Disbursement Value: ₹1");
      return;
    }
    ...
    try {
      ...
      alert("Disbursement Protocol Failure: " + (err.response?.data?.message || "Internal Server Error"));
    } finally {
      setProcessing(false);
    }
  };
                ...
                <div className="relative z-10 max-w-2xl mx-auto space-y-10">
                  <div className="text-center">
                    <div className="inline-flex items-center gap-2 px-4 py-2 bg-amber-50 rounded-full border border-amber-100 mb-4">
                      ...
                      <span className="text-[10px] font-black uppercase tracking-widest text-amber-600 italic">Liquid Balance: ₹{data.balance.toLocaleString()}</span>
                    </div>
                    <h2 className="text-3xl font-black text-slate-900 tracking-tighter uppercase italic">Initialize Disbursement</h2>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-2">Enter banking nodes for institutional transfer</p>
                  </div>

                  <form onSubmit={handleWithdraw} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="md:col-span-2 space-y-2">
                      <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest italic ml-4">Transmission Value (₹)</label>
                      <input 
                        type="number" 
                        required
                        placeholder="0.00"
                    ...
                    <div className="space-y-2">
                      <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest italic ml-4">Banking Node</label>
                      <input 
                        type="text" 
                        required
                    ...
                    <div className="space-y-2">
                      <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest italic ml-4">Account Identification</label>
                      <input 
                        type="text" 
                        required
                    ...
                    <div className="md:col-span-2 space-y-2">
                      <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest italic ml-4">IFSC Protocol</label>
                      <input 
                        type="text" 
                        required
                  ...
                  <AnimatePresence>
                    {success && (
                      ...
                      >
                        ...
                        <div>
                          <p className="text-[10px] font-black text-emerald-900 uppercase italic tracking-widest">Protocol Accepted</p>
                          <p className="text-[9px] font-bold text-emerald-600 uppercase tracking-widest mt-1">Disbursement request has been logged in the audit ledger.</p>
                        </div>
                      </motion.div>
                    )}
                  </AnimatePresence>
            ...
            <div className="space-y-6">
              <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white space-y-6 shadow-xl">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="font-black text-sm uppercase italic tracking-tight">Security Matrix</h3>
                </div>
                <div className="space-y-4">
                  {[
                    "All disbursements undergo 2FA validation.",
                    "Processing time: 2-24 business hours.",
                    "Institutional limits apply based on KYC level.",
                    "Verify bank identification before execution."
                  ].map((note, i) => (
              ...
              <div className="bg-amber-50 border border-amber-100 rounded-[2.5rem] p-8 space-y-4">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="font-black text-[10px] text-amber-900 uppercase tracking-widest italic">Live Processing</h3>
                </div>
                <p className="text-[9px] font-bold text-amber-700 leading-relaxed italic">
                  Node status: <span className="text-emerald-600">Operational</span>. Multi-sig validation required for transfers exceeding ₹50,000.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-[3rem] border border-slate-200 overflow-hidden shadow-sm">
            <div className="p-8 border-b border-slate-100 flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-lg font-black text-slate-900 uppercase italic tracking-tighter">Disbursement Audit</h3>
                  <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest italic">Live Liquidation Logs</p>
                </div>
              </div>
            </div>
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="bg-slate-50">
                    ...
                    <th className="px-8 py-5 text-left text-[9px] font-black text-slate-400 uppercase tracking-widest italic">Banking Node</th>
                  ...
                  {data.withdrawals.length === 0 && (
                    <tr>
                      <td colSpan="5" className="px-8 py-20 text-center">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic">Zero Active Liquidation Events Detected</p>
                      </td>
                    </tr>
                  )}
                </tbody>
