...
const LandingPage = () => {
  ...
  const faqs = [
    ...
    { q: "What is the referral hierarchy?", a: "We operate a 5-tier referral matrix: Level 1 (0.5%), Level 2 (0.02%), Level 3 (0.02%), Level 4 (0.01%), and Level 5 (0.01%) commissions." },
        ...
        >
          {[...Array(8)].map((_, i) => (
            <span key={i} className="flex items-center gap-16">
              <span className="flex items-center gap-4 text-amber-400"><TrendingUp size={14} strokeWidth={3}/> LIVE TICKER: ₹{parseFloat(stats.currentGoldRate).toLocaleString()}/g (24K PURE)</span>
           ...
           <div className="fixed inset-0 z-[100] lg:hidden">
              ...
              <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 25, stiffness: 200 }} className="absolute right-0 top-0 bottom-0 w-[85%] bg-white shadow-2xl flex flex-col p-8">
                 ...
                 <div className="space-y-4 pt-12 border-t border-slate-100">
                    <Link to="/login" onClick={() => setIsMenuOpen(false)} className="w-full py-6 bg-white border border-slate-200 text-slate-900 rounded-[1.8rem] flex items-center justify-center font-black uppercase text-[11px] tracking-widest italic">Login Terminal</Link>
                    <Link to="/register" onClick={() => setIsMenuOpen(false)} className="w-full py-6 bg-slate-900 text-white rounded-[1.8rem] flex items-center justify-center font-black uppercase text-[11px] tracking-widest italic shadow-xl">Create Identity</Link>
                 </div>
              </motion.div>
           </div>
        )}
