...
- **Combined Earnings Cap (Principal Lock)**: A member's **cashback + referral earnings together** are capped at **100% of their principal** (ex-GST purchase value). The engine tracks the combined total against the cap and **stops all payouts the moment it is reached — even before the 100-day cycle completes**. Correctness is guaranteed by re-reading each cycle's *live* `paid_amount` before crediting (so referral commissions credited earlier in the same pass are always counted), making over-payment beyond the principal impossible. The customer Dashboard surfaces an **Earnings Cap** panel (earned vs. principal vs. remaining, with a completion state) that updates live.
- **Configurable TDS & Service Charges**: Every incentive credit (daily cashback **and** referral commission) has an admin-tunable **TDS %** and **service/processing charge %** withheld automatically. The customer's cycle still accrues the **gross** amount (so the 1%/100-day/principal-cap math is untouched) while the wallet is credited the **net**. Each ledger row stores the full breakdown — `gross_amount`, `tds_amount`, `charges_amount`, `deduction` (total) and `amount` (net) — surfaced as a deduction breakdown in both the **Customer** (Transaction History, Referral Network, Dashboard) and **Admin** (Reports → Cashback / Referral, Wallet Adjustment) panels, all live.
- **Referral Stop / Resume Control**: Admins can **pause a specific member's referral commissions** with one click from the Users grid (`users.referral_active` flag). The yield engine skips paused members for every referral payout while their own daily cashback continues unaffected; the member sees a live "Referral Commissions Paused" banner on their Referral Network page and receives a notification.
- **Cashback Application on Checkout**: Right after a customer places an order, a **Cashback Application popup** opens — prefilled with their live customer + bank details and the just-placed purchase (amount, product, date) — for the customer to complete and submit manually, persisting to the `cashback_applications` table.
...
- **Two-Way Feedback & Remarks**: A bidirectional communication channel backed by a live `feedbacks` table. Customers submit feedback with a star rating and read replies from the team via an embedded widget on their Dashboard and Profile; the admin/staff **Feedback & Remarks** console shows the customer inbox, lets operators send remarks to any specific customer (which also drops a notification for them), and keeps a sent history. Both sides poll every 20 seconds so threads stay live.
- **Festival Offers & Login Promotions**: An admin **Offers & Festivals** console publishes promotional/festival banners (title, message, badge, pick-able accent colour, optional validity window) into an `offers` table. Active offers surface automatically as a **popup right after login** and as a **banner on the login page** — created, toggled, and expired in real time with no redeploy.
- **VEV Member Identity Everywhere**: The platform's `VEV###` member IDs are surfaced consistently across the suite — the customer Profile ("Member ID"), the admin Users grid (each account's own ID plus a **Referred By** upline name + ID), the Advocate customer directory, and the Referral network (direct members and the multi-level downline tree) — all served live from the MySQL core.
- **Blue-Navy + Gold Design System**: The entire suite is themed on a white base with **deep navy (`blue-900`)** surfaces/headings, a **vivid blue (`blue-600`)** primary action colour, and **gold (amber)** accents — applied uniformly across the landing, authentication, admin, and customer interfaces, with a fully **mobile-responsive** pass across every admin console and the public landing page.
...
- **TailwindCSS**: Utility-first styling for a premium, custom-branded navy-blue + gold design system (white base, `blue-900` surfaces, `blue-600` primary, amber accents), fully mobile-responsive.
...
│   ├── src/                
│   │   ├── components/     # Reusable UI Frameworks (Header, Sidebar, FeedbackWidget, AdminFeedback, AdminOffers, CashbackApplicationModal)
