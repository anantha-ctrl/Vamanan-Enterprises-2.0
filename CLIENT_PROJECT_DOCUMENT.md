...
### Phase 12: Yield Economics — Deductions, Principal Cap & Referral Governance
- **Live Landing Metrics Engine**: Rebuilt the public landing headline stats (Institutional Partners, Managed Assets, Weekly Payouts) to serve **live from `api/public/stats.php`** — real platform counts plus admin-tunable **marketing offsets** (`marketing_investor_offset / capital / payout`), rendered as CountUp tiles that grow automatically as real volume accrues.
- **Configurable TDS & Service Charges**: Added two independent, admin-configurable deduction rates — **TDS %** (`tds_rate`) and **Service / Processing Charges %** (`service_charge_rate`) — withheld automatically from every daily-cashback **and** referral credit. The member's cycle still accrues the **gross** (so all cap math is untouched) while the wallet is credited the **net**; each ledger row persists `gross_amount`, `tds_amount`, `charges_amount`, and `deduction`. The full breakdown is surfaced in real time across the **Customer** panels (Transaction History, Referral Network, Dashboard "Daily Payout (Net)") and the **Admin** panels (Reports → Cashback/Referral **Deduction Breakdown**, and the **Wallet Adjustment** final-net-to-customer summary).
- **Combined Earnings Cap (Principal Lock)**: Enforced that a member's **cashback + referral earnings together** cap at **100% of their principal** (ex-GST purchase value) and that **all payouts stop the instant the cap is reached — even before the 100-day cycle ends**. Fixed a stale-read defect in the engine (cycles were bulk-fetched once per pass, so referral commissions credited earlier in the same pass were ignored by the cashback step, allowing over-payment past the principal); the cashback step now re-reads each cycle's **live** `paid_amount` before crediting. Added an **Earnings Cap** panel to the customer Dashboard (earned vs. principal vs. remaining + completion state). Verified end-to-end that combined credits never exceed the principal.
- **Daily-Payout Correctness**: Fixed cycles being stored with `daily_payout = 0` (the panel showed ₹0 while the engine actually paid on the ex-GST base). Cycles now store `daily_payout = 1% of the ex-GST base` at purchase, (re)compute it on **Authorize Activation**, and the admin **Wallet Adjustment** panel computes it **live** (`get_user_daily_payout.php`) — showing gross, TDS, charges, and the **final net amount credited to the customer**, with the manual "Apply Net Daily Payout" writing the net.
- **Direct Referral Rate → 1%**: Set the direct (Level-1) referral commission to **1%** of the referred member's ex-GST purchase (admin-configurable; L2–L5 retained).
- **Referral Stop / Resume Governance**: Gave admins one-click control to **pause a specific member's referral commissions** (`users.referral_active`, `api/admin/toggle_referral.php`). The yield engine skips paused members for all referral payouts while their own daily cashback continues; the Users grid shows a live **● Earning / ■ Stopped** pill with a Stop/Resume button, and the member sees a "Referral Commissions Paused" banner plus a notification.
- **Cashback Application at Checkout**: After a customer places an order, a **Cashback Application popup** (`CashbackApplicationModal.jsx`) opens prefilled with their live profile + bank details and the just-placed purchase (amount, product, date) for manual completion and submission to `cashback_applications`.
